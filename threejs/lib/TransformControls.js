( function () {
	...
	class TransformControls extends THREE.Object3D {
		...
		pointerMove( pointer ) {
			...
			// 更新Thetalength
			// 跟新sectorMesh
			this._gizmo.sectorNormal = this.rotationAxis;
			this._gizmo.sectorThetaLength = this.rotationAngle;
			this._gizmo.updateSectorMesh();
		...
		pointerUp( pointer ) {
			...
			this.axis = null;
			// 跟新 sectorMesh
			this._gizmo.sectorThetaStart = 0;
			this._gizmo.sectorThetaLength = 0;
			this._gizmo.updateSectorMesh();
	...
	class TransformControlsGizmo extends THREE.Object3D {
		...
		constructor() {
			...
			// customDarkGray
			const customGray = gizmoMaterial.clone();
			customGray.color.setHex( 0xffffff );
			customGray.opacity = 0.5;
			const customDarkGray = gizmoMaterial.clone();
			customDarkGray.color.setHex( 0x666666 );
			customDarkGray.opacity = 0.3;
			...
			// 扇形区域
			this.sectorMesh = null;
			this.sectorThetaStart = 0;
			this.sectorThetaLength = Math.PI / 2;
			this.sectorNormal = new THREE.Vector3(0, 0, 1);
			...
			this.sectorMesh = this.getSectorMesh();
			...
			const gizmoRotate = {
				XYZE: [[ new THREE.Mesh( CircleGeometry( 0.5, 1 ), customDarkGray ), null, [ 0, Math.PI / 2, 0 ]]],
				...
				E: [[ new THREE.Mesh( CircleGeometry( 0.75, 1 ), customGray), null, [ 0, Math.PI / 2, 0 ]]],
				// 自定义坐标轴
				AXIS: [[ this.sectorMesh ] ]
			};
			const helperRotate = {
				AXIS: [[AixsMesh()]]
		...
		// 获取扇形
		getSectorMesh(thetaStart = 0, thetaLength = 0, normal = new THREE.Vector3(0, 0, 1), radius=0.48, segments = 50) {
			...
			const geomtry = new CustomSectorGeometry(thetaStart, thetaLength, normal, radius, segments);
		...
		updateSectorMesh() {
			...
			if (this.sectorNormal.x) {
				color.setHex(0xff0000);
			}
			if (this.sectorNormal.y) {
				color.setHex(0x00ff00);
			}
			if (this.sectorNormal.z) {
				color.setHex(0x0000ff);
			}
			this.sectorMesh.geometry && this.sectorMesh.geometry.dispose();
			this.sectorMesh.geometry = new CustomSectorGeometry(this.sectorThetaStart, this.sectorThetaLength, this.sectorNormal);
			this.sectorMesh.material.color = color;
			...
			for ( let i = 0; i < handles.length; i ++ ) {
				...
				if ( this.enabled && this.axis ) {
					...
					if ( handle.name === this.axis ) {
						...
						handle.material.color.setHex( 0xFFC200 );
